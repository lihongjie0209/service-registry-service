...
平台内部的 Redis 租约型服务注册中心。它提供实例注册、续租、draining、注销、按服务名或元数据发现、revision 断点续看和管理端 POST+JSON 查询接口；不替代 Kubernetes Service/DNS，也不实现自有共识协议。公共 Go 客户端位于 `microservice-platform-go/serviceregistry`。
...
```bash
make docker-build
docker inspect service-registry-service:$(git describe --tags --always --dirty) \
...
After unit and integration tests pass, CI builds `linux/amd64` and `linux/arm64` images and publishes `main`, `sha-*`, and semantic-version tags to `ghcr.io/lihongjie0209/service-registry-service`. Images include Git metadata, an SBOM, and build provenance. Pull requests build both architectures without publishing.
...
kubectl create secret generic service-registry-service --namespace platform-system \
...
kubectl wait --namespace platform-system --for=condition=complete job/service-registry-service-migrate --timeout=5m
...
Never reuse deleted field numbers or names: mark them `reserved`. Introduce breaking contracts under a new package such as `orders.v2`, and keep `v1` available until consumers have migrated. The module name in this template is `buf.build/lihongjie0209/service-registry-service`; publishing it requires a matching BSR account/module and a `BUF_TOKEN` secret. For an internal installation, replace the module name with the organization's BSR module.
