...
import (
	...
	"github.com/lihongjie0209/service-registry-service/internal/app"
...
func TestHTTPAndGRPCEndToEnd(t *testing.T) {
	...
	cfg := config.Config{
		...
		Auth:          config.Auth{ClientID: "client", ClientSecret: "secret", SkipHTTPPaths: []string{"/api/v1/version"}, SkipGRPCMethods: []string{"/grpc.health.v1.Health/*"}, PSK: config.PSK{Enabled: true, Key: secret, GRPCMethods: []string{"/platform.registry.v1.RegistryService/*"}}},
	...
	if _, err := registryv1.NewRegistryServiceClient(connection).RegisterInstance(pskCtx, &registryv1.RegisterInstanceRequest{Instance: &registryv1.ServiceInstance{ServiceName: "integration-service", InstanceId: "pod-1", Endpoint: "grpc://integration-service:9090"}, LeaseSeconds: 30}); err != nil {
		t.Fatalf("PSK RegisterInstance: %v", err)
	}
}
